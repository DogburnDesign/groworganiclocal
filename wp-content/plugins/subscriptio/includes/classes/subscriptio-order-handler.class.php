...
class Subscriptio_Order_Handler
{
    ...
    public function new_order($order_id)
    {
        ...
        foreach ($woocommerce->cart->cart_contents as $cart_item_key => $cart_item) {
            ...
            if (Subscriptio_Subscription_Product::is_subscription($id)) {
                ...
                $product = Subscriptio::wc_version_gte('2.2') ? wc_get_product($id) : get_product($id);
                ...
                foreach($renewal_cart_item_keys as $renewal_cart_item_key) {
                    ...
                    $renewal_order['renewal_order_discount'] = wc_format_decimal(Subscriptio::wc_version_gte('2.2') ? $renewal_cart->get_total_discount() : $renewal_cart->get_order_discount_total());
    ...
    public function new_order_placed($order_id, $posted)
    {
        ...
        foreach ($order_items as $order_item_key => $order_item) {
            ...
            $product_post_meta = Subscriptio::unwrap_post_meta(get_post_meta($variation_id ? $variation_id : $product_id));
        ...
        if (!empty($all_subs)) {
            ...
            for ($i = 0; $i < $iterations; $i++) {
                ...
                $product = Subscriptio::wc_version_gte('2.2') ? wc_get_product($product_id) : get_product($product_id);
    ...
    public static function create_renewal_order($subscription)
    {
        ...
        for ($i = 0; $i < $iterations; $i++) {
            ...
            if (Subscriptio::product_is_active($product_id_to_use)) {
                ...
                // Load product object
                $product = Subscriptio::wc_version_gte('2.2') ? wc_get_product($product_id_to_use) : get_product($product_id_to_use);
            ...
            foreach ($subscription->renewal_all_items_meta[$product_id_to_use] as $item_meta_key => $item_meta_value) {
    ...
    public function order_refunded_maybe_cancel_subscription($refunded_order_id)
    {
        ...
        foreach (self::get_subscriptions_from_order_id($refunded_order_id) as $subscription) {
            ...
            foreach ($subscription->all_order_ids as $subscription_order_id) {
                ...
                $order = new WC_Order($subscription_order_id);
    ...
    public function maybe_prevent_file_download($email, $order_key, $product_id, $user_id, $download_id, $order_id)
    {
        ...
        if (!Subscriptio_User::has_access_to_product_downloads($product_id)) {
            ...
            // Add notice
            Subscriptio::add_woocommerce_frontend_error(__('You no longer have access to this file.', 'subscriptio'));
