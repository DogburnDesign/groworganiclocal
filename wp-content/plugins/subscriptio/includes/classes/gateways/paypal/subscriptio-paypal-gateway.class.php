...
class Subscriptio_PayPal_Gateway extends WC_Payment_Gateway
{
    ...
    public function send_request($action, $params)
    {
        ...
        if (is_wp_error($result)) {
            ...
            // Write error to log
            self::log_write($action . ' request', $result->get_error_message());
        ...
        // Result is empty
            ...
            self::log_write($action . ' request', 'empty response');

            // Return it
            return __('Response body empty.', 'subscriptio-paypal');
        }

        // Result can be normally processed
        $result_decoded = json_decode($result['body']);
            ...
            if (!is_array($paypal_log)) {
                ...
                foreach ($paypal_log as $entry) {
                    ...
                    if (!empty($entry)) {
                        ...
                        if (!empty($object)) {
                            echo ', details:<br><pre>' . $object . '</pre>';
                        }
                    }
                }
            }
